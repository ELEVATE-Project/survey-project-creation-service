...
const isSunbird = process.env.CONSUMPTION_SERVICE == common.SUNBIRD
...
async function getDefaultUserId() {
	...
	if (orgDetails.success && process.env.CONSUMPTION_SERVICE == common.SUNBIRD) {
		const userSearchUrl = process.env.USER_SERVICE_HOST + endpoints.SUNBIRD.USER_SEARCH
		const userSearchBody = {
			request: {
				filters: {
					'roles.role': common.ORG_ADMIN_ROLE.toUpperCase(),
					'organisations.organisationId': orgDetails?.result?.id,
				},
				limit: 1,
			},
		}

		const fetchOrgAdmin = await requests.post(userSearchUrl, userSearchBody, process.env.SUNBIRD_X_AUTH_TOKEN)
		defaultUserId = fetchOrgAdmin?.data?.result?.response?.content?.[0]?.id || ''
	} else if (
		orgDetails.success &&
			...
			for (const entity of entitiesToCreate) {
				if (entity.value) {
					...
					if (createdEntity?.result?.id) {
						...
						entitiesToCreate = entitiesToCreate.filter(
							(en) => en.entity_type_id != entity.entity_type_id && en.value != entity.value
						)
						console.error(
							`Entity already Exists OR Failed to create entity: ${entity.value}`,
							createdEntity.error || createdEntity
						)
					}
				}
			}
...
async function createProgram(programId, programData, userId, orgId, solutionIds) {
	try {
		...
		//update and publish resource
		const updatePayload = {
			published_id: programId.toString(),
			published_on: new Date(),
			status: common.RESOURCE_STATUS_PUBLISHED,
			stage: common.RESOURCE_STAGE_COMPLETION,
		}

		let updateResourceRes = await updateResource(createProgramRes.result.id, updatePayload)
		if (!updateResourceRes.success) {
			console.error('Failed to create program ', programId)
...
async function generateTargetingCriteria(scope = {}, db) {
	try {
		...
		} else if (!isSunbird && entityIds.length) {
			entityIds = entityIds.length ? entityIds.map((id) => ObjectId(id)) : []
			entitesFindQuery = { _id: { $in: entityIds } }
			entityTypesFindQuery = { name: { $in: entityTypeIds } }
		}
 ...
 * @param {Object} entityId - entity id
