		...
		for (const chunk of chunkedTemplates) {
			...
			for (const template of templates) {
				...
				if (!Array.isArray(template.tasks) || template.tasks.length === 0) {
					console.log(`No task found for template ${templateIdStr}`)
					csvRecords.push({
						templateId: templateIdStr,
...
//get user org id
async function getUserOrgDetails(userIds, userToken) {
	let userOrgMap = {}
...
async function createProject(templateId, projectData, userId, orgId) {
	try {
		...
		const updatePayload = {
			published_id: templateId.toString(),
			published_on: new Date(),
			status: common.RESOURCE_STATUS_PUBLISHED,
			stage: common.RESOURCE_STAGE_COMPLETION,
		}

		let updateResourceRes = await updateResource(createProject.result.id, updatePayload)
