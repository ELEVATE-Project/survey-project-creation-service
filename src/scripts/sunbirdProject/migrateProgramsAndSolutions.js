...
const userToken = process.env.SUNBIRD_X_AUTH_TOKEN
...
;(async () => {
	try {
		...
		for (const chunk of chunkedPrograms) {
			...
			let userOrgMap = {}
			if (userIds.length > 0) {
				userOrgMap = await getUserOrgDetails(userIds, userToken)
			}
		...
		if (entitiesToCreate.length > 0) {
			for (const entity of entitiesToCreate) {
				if (entity.value) {
					...
					const createdEntity = await entityService.create(entityCreationData, '0', DEFAULT_ORG_ID)
